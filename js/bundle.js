...
function cards() {
 //Cards (импользуем для карточек классы)
...
function openModal(modalSelector, modalTimerId) {
    ...
    if (modalTimerId) {
        clearInterval(modalTimerId); // если пользователь сам открыл мод.окно, то очистим этот интервал, чтобы через некоторое время окно больше не открывалось
    }
   }
...
function slider ({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
 //Slider
    const slides = document.querySelectorAll(slide),
            ...
            width = window.getComputedStyle(slidesWrapper).width; // ширина окошка через которое виден слайд (ширина слайда)

    let slideIndex = 1;

    let offset = 0; // отступ на скко отступмлм
    ...
    slidesField.style.width = 100 * slides.length + '%' ; // 100 % умнож на колво слайдов - это процент тк это добавляем в css стили , а там нужна единицы измерения
...
slidesWrapper.style.overflow = 'hidden'; // скрываем лишние слайды
    ...
    const indicators = document.createElement('ol'), // создаем переменную для точек-индикаторов
            dots = [];
    ...
    for(let i = 0; i < slides.length; i++) {  //создаем определенное колво точек
        const dot = document.createElement('li');
        dot.setAttribute('data-slide-to', i + 1); //создаем арибут к каждой точек принадлежит определенный по счету слайд
        dot.style.cssText = `
    ...
    if (offset == deleteNotDigits(width) * (slides.length - 1)) { //в width  лежит условно строка - условно "500рх"  и мы удаляем буквы "рх" с помощью регулярных вырвжений
        offset = 0;
    } else {
        offset += deleteNotDigits(width);// если слайд не последний, то прибавляем еще ширину одного слайда(те смещаем влевовесь поток слайдов)
    }
    slidesField.style.transform = `translateX(-${offset}px)`; // двигаем по оси Х 
...
dots.forEach(dot => {
    ...
    /* showSlides(slideIndex);  // слайдер 2 вар, данные , их получение к слайду-выше
